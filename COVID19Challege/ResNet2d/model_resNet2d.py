...
class ResNet2dModule(object):
    ...
    def train(self, train_images, train_lanbels, val_images, val_labels, model_path,
              logs_path, learning_rate, dropout_conv=0.8, train_epochs=5, batch_size=1):
        ...
        train_images_data = np.empty((len(train_images), self.image_height, self.image_width, self.channels))
        for num in range(len(train_images)):
            batchimage = np.reshape(cv2.imread("dataprocess\\" + train_images[num], 0),
        ...
        train_images_data = np.multiply(train_images_data, 1.0 / 255.0)

        val_labels_onehot = dense_to_one_hot(val_labels, label_counts)
        val_labels_onehot = val_labels_onehot.astype(np.float)
        val_images_data = np.empty((len(val_images), self.image_height, self.image_width, self.channels))
        for num in range(len(val_images)):
            batchimage = np.reshape(cv2.imread("dataprocess\\" + val_images[num], 0),
                                    (self.image_height, self.image_width, self.channels))
            val_images_data[num, :, :, :] = batchimage
        # Extracting images and labels from given data
        val_images_data = val_images_data.astype(np.float)
        # Normalize from [0:255] => [0.0:1.0]
        val_images_data = np.multiply(val_images_data, 1.0 / 255.0)
        ...
        for i in range(train_epochs):
            # get new batch
            batch_xs, batch_ys, index_in_epoch = _next_batch(train_images_data, train_labels_onehot, batch_size,
                                                             index_in_epoch)
            # Extracting images and labels from given data
            ...
            if i % DISPLAY_STEP == 0 or (i + 1) == train_epochs:
                ...
                validataion_loss, validataion_accuracy = self.sess.run([self.cost, self.accuracy],
                                                                       feed_dict={self.X: val_images_data,
                                                                                  self.Y_gt: val_labels_onehot,
                                                                                  self.lr: learning_rate,
                                                                                  self.drop: 1,
                                                                                  self.phase: 1})
                print('epochs %d training_loss ,training_accuracy=> %.5f,%.5f ' % (i, train_loss, train_accuracy))
                print('epochs %d validation_loss,validation_accuracy => %5f,%5f ' % (
                    i, validataion_loss, validataion_accuracy))
